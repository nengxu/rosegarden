...
{ 
	...
	if (*mi == Rosegarden::Marks::Pause ||
	    ...
	    *mi == Rosegarden::Marks::LongTrill ||
	    *mi == Rosegarden::Marks::TrillLine ||
			     ...
			     int stemLength)
{
	...
	if (!Rosegarden::Marks::isFingeringMark(*i)) {
	    ...
	    if (*i != Rosegarden::Marks::TrillLine) {
                                   ...
                                   int thickness, bool smooth)
{
    ...
    if (colours.size() == 0) {
	int h, s, v;
        QColor c = Rosegarden::GUIPalette::getColour(Rosegarden::GUIPalette::SelectedElement);
c.hsv(&h, &s, &v);
...
QCanvasPixmap *
NotePixmapFactory::makeMarkMenuPixmap(Rosegarden::Mark mark)
{
    if (mark == Rosegarden::Marks::Sforzando ||
	mark == Rosegarden::Marks::Rinforzando) {
				 ...
				 QPainter *painter, int x, int y)
{
    ...
    if (m_selected) {
	m_p->painter().setPen(Rosegarden::GUIPalette::getColour(Rosegarden::GUIPalette::SelectedElement));
	pen.setColor(Rosegarden::GUIPalette::getColour(Rosegarden::GUIPalette::SelectedElement));
    } else if (m_shaded) {
	m_p->painter().setPen(Qt::gray);
			       ...
			       QPainter *painter, int x, int y)
{
    ...
    if (m_selected) m_p->painter().setPen(Rosegarden::GUIPalette::getColour(Rosegarden::GUIPalette::SelectedElement));
