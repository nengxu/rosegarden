...
SegmentSplitByRecordingSrcCommand::execute()
{
    if (!m_newSegmentA) {
        ...
        for (Segment::iterator i = m_segment->begin();
            ...
            if ((*i)->has(Rosegarden::BaseProperties::RECORDED_CHANNEL)) {
                selectedC = true;
                if (m_channel > -1)
                    selectedC = ( m_channel == 
                        (*i)->get<Rosegarden::Int>(Rosegarden::BaseProperties::RECORDED_CHANNEL) );
            }

            if ((*i)->has(Rosegarden::BaseProperties::RECORDED_PORT)) {
                selectedD = true;
                if (m_device > -1) 
                    selectedD = ( m_device == 
                        (*i)->get<Rosegarden::Int>(Rosegarden::BaseProperties::RECORDED_PORT) );
            }
...
OpenOrCloseRangeCommand::execute()
{
	...
	if (!m_opening) movingFrom = m_endTime;
	...
	for (Rosegarden::TempoSelection::tempocontainer::const_iterator i = 
	    ...
	    timeT t = i->first;
	    Rosegarden::TempoSelection::tempochange change = i->second;
