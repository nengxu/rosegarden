...
TextEventDialog::slotTypeChanged(const QString &)
{
    ...
    else if (type == Text::Chord) m_text->setText(m_prevChord);
    else if (type == Text::Annotation) m_text->setText(m_prevAnnotation);
...
TextEventDialog::slotOK()
{
    ...
    if (index == 5) config->writeEntry("previous_chord", m_text->text());
    else if (index == 6) config->writeEntry("previous_lyric", m_text->text());
    else if (index == 7) config->writeEntry("previous_annotation", m_text->text());
}
...
TempoDialog::populateTempo()
{
    ...
    std::pair<bool, Rosegarden::tempoT> ramping(false, tempo);
