...
void Barre::load ( QDomNode const& node )
{
    if ( ! node.hasAttributes() ) {
        throw Exception ( "Barre::load - Input node has no attributes" );
    }

    QDomNamedNodeMap attributes = node.attributes();
    QDomNode attrNode = attributes.namedItem( "fret" );
    if ( attrNode.isNull() ) {
        throw Exception ( "Barre::load - Input node missing required attribute (fret)" );
    }
    ...
    if ( attrNode.isNull() ) {
        throw Exception ( "Barre::load - Input node missing required attribute (start)" );
    }
    ...
    if ( attrNode.isNull() ) {
        throw Exception ( "Barre::load - Input node missing required attribute (end)" );
    }
