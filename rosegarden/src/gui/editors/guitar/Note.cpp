...
const PropertyName Note::NOTE_FRET = "note_fret";
const PropertyName Note::NOTE_STRING = "note_string";
...
void Note::load ( QDomNode const& node, GuitarNeck* g_ptr )
{
    if ( ! node.hasAttributes() ) {
        throw Exception ( "Note::load - Input node missing attributes" );
    }

    QDomNamedNodeMap attributes = node.attributes();
    QDomNode attrNode = attributes.namedItem( "string" );
    if ( attrNode.isNull() ) {
        throw Exception ( "Note::load - Input node missing attribute (string)" );
    }
