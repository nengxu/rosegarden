...
// TransactionLockable can be locked by one client at once, and issues
// an id for each lock which other methods can authenticate against
// with an isGood() call.  This means an operation can be associated
// with a particular transaction, and mistakes (such as clients
// attempting to carry out an operation when another transaction is
// going on) reduced.
// 
// In the Rosegarden Database, the Composition is a TransactionLockable
// and every mutating operation has to show a valid transaction id.
// (database/impl/Transaction.[Ch] wrap this class; they also admit the
// possibility of null id for clients that are unable for threading
// reasons to respect locks, or for operations that are not part of a
// transaction.)
