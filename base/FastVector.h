...
// ==========
// 
// This is a sequence class with an interface similar to that of the
// STL vector, with several nice properties and one nasty one:
// 
...
public: 
    ...
    // We *guarantee* that push methods etc modify the
    // FastVector only through a call to insert(size_type, T),
    // and that erase etc modify it only through a call to
    // remove(size_type).  This is important because subclasses only
    // need to override those functions to catch all mutations
...
public:
	...
	// This is what makes begin() and end() work -- we can't override
	// them, as they don't return a ref so we'd need covariant return
