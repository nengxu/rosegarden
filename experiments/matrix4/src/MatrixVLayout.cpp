...
void MatrixVLayout::resetStaff(AbstractViewElementManager&, timeT, timeT)
{}

void MatrixVLayout::scanStaff(AbstractViewElementManager& viewElementManager,
                              LinedStaff &staff,
                              timeT startTime, timeT endTime)
{
    MatrixViewElementManager& matrixViewElementManager = dynamic_cast<MatrixViewElementManager&>(viewElementManager);
    MatrixStaff& matrixStaff = dynamic_cast<MatrixStaff&>(staff);
...
//    << matrixViewElementManager.getId() << endl;
    ...
    for (i = from; i != to; ++i) {
        ...
        int y = matrixStaff.getLayoutYForHeight(pitch) - matrixStaff.getElementHeight() / 2;

        el->setLayoutY(y);
        el->setHeight(matrixStaff.getElementHeight());
    }
