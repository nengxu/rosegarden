...
LinedStaff::LinedStaff(QGraphicsScene *canvas, Segment *segment,
                       SnapGrid *snapGrid, AbstractViewElementManager *viewElementManager, int id,
        ...
        m_endLayoutX(0),
        m_current(false),
        m_pointer(new QGraphicsLineItem(0)),
        m_insertCursor(new QGraphicsLineItem(0))
{
    initCursors();
}

LinedStaff::LinedStaff(QGraphicsScene *canvas, Segment *segment,
                       SnapGrid *snapGrid, AbstractViewElementManager *viewElementManager,
...
LinedStaff::initCursors()
{
    ...
    m_canvas->addItem(m_pointer);
    m_canvas->addItem(m_insertCursor);
}
...
LinedStaff::getBarExtents(double x, int y) const
{
    ...
    for (int i = 1; i < m_barLines.size(); ++i) {
        ...
        if (line) {
            ...
            return QRectF(m_barLines[i -1]->x(),
                         getCanvasYForTopOfStaff(barRow),
                         line->x() - m_barLines[i - 1]->x(),
                         getHeightOfRow());
        }
    }

    // failure
    return QRectF(getX() + getMargin(), getCanvasYForTopOfStaff(), 4, getHeightOfRow());
}
                      ...
                      int barNo, bool showBarNo)
{
    ...
    if (lastBarInRow) {
        ...
        BarLine *eline = new BarLine(layoutX,
                                     getBarLineHeight(), barThickness, getLineSpacing(),
                                     0, style);
        eline->moveBy(xe, y);
    ...
    */
    ...
    if (m_pageMode != LinearMode && m_connectingLineLength > 0.1) {
        ...
        QGraphicsLineItem *line = new QGraphicsLineItem
                                 (x + length, y, x + length, y + m_connectingLineLength);
        
        QPen pen = GUIPalette::getColour(GUIPalette::StaffConnectingTerminatingLine);
    ...
    for (h = 0; h < getLineCount(); ++h) {
        ...
        QGraphicsLineItem *lline = new QGraphicsLineItem(x, y, x + length, y);
        QPen pen(lineColour);
...
LinedStaff::setPointerPosition(double canvasX, int canvasY)
{
    ...
    m_pointer->setPos(canvasX, canvasY);
...
LinedStaff::setInsertCursorPosition(double canvasX, int canvasY)
{
    ...
    int row = getRowForCanvasCoords(canvasX, canvasY);
    canvasY = getCanvasYForTopOfStaff(row);
    m_insertCursor->setPos(canvasX, canvasY);
