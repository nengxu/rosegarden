...
// class DevicesManagerNew : public Ui::DevicesManagerNewUi, public QDialog
	...
	void DevicesManagerNew:: slotRefreshOutputPorts( ){
		...
		updateDevicesList( 0, m_treeWidget_playbackDevices, MidiDevice::Play );
		
		updateCheckStatesOfPortsList( m_treeWidget_outputPorts, m_treeWidget_playbackDevices );
	}
	...
	void DevicesManagerNew:: slotRefreshInputPorts( ){
		...
		updateDevicesList( 0, m_treeWidget_recordDevices, MidiDevice::Record );
		
		updateCheckStatesOfPortsList( m_treeWidget_inputPorts, m_treeWidget_recordDevices );
	}
	...
	// slot
	void DevicesManagerNew:: slotOutputPortClicked( QTreeWidgetItem * twItem, int column ){
		//
	...
	// slot
	void DevicesManagerNew:: slotInputPortClicked( QTreeWidgetItem * twItem, int column ){
		//
		//### updating the devices should not happen here
		//### this is a bad fix, that makes it work 50% of time
 		updateDevicesList( 0, m_treeWidget_recordDevices, MidiDevice::Record );
	...
	// slot
	void DevicesManagerNew:: slotOutputPortDoubleClicked( QTreeWidgetItem * twItem, int column ){
		MidiDevice* mdev;
		QString portName;
	...
	// slot
	void DevicesManagerNew:: slotInputPortDoubleClicked( QTreeWidgetItem * twItem, int column ){
		MidiDevice* mdev;
		QString portName;
	...
	QTreeWidgetItem*  DevicesManagerNew:: searchItemWithPort( QTreeWidget* treeWid, QString portName ){
	...
	QTreeWidgetItem*  DevicesManagerNew:: searchItemWithDeviceId( QTreeWidget* treeWid, DeviceId devId ){
	...
	void DevicesManagerNew:: updateDevicesList ( 
			DeviceList* devices,
			QTreeWidget* treeWid, 
...
// 		cnt = m_treeWidget_playbackDevices->topLevelItemCount();
		...
		for ( i=0; i < cnt; i++ )
		{
			...
			if ( mdev->getDirection() == in_out_direction )
			{
				...
				else	// list contains mdev (midi-device)
				{
					...
					{
						RG_DEBUG << "Warning: twItem is NULL in DevicesManagerNew::updateDevicesList() " << endl;
					}
				}// if contains
	...
	MidiDevice* DevicesManagerNew:: getCurrentlySelectedDevice( QTreeWidget* treeWid ){
		...
		twItem = 0;
		if( twItemsSelected.count() >= 1 ) twItem = twItemsSelected.at(0);
...
// 					twItemSelected = twItem;
	...
	void DevicesManagerNew::connectMidiDeviceToPort( MidiDevice* mdev, QString portName )
	{
		if( ! mdev ){
			RG_DEBUG << "Warning: mdev is NULL in DevicesManagerNew::connectPlaybackDeviceToOutputPort() " << endl;
			return;
		}
...
// 		m_recordDevices.clear();
	...
	void DevicesManagerNew:: updatePortsList( QTreeWidget* treeWid, MidiDevice::DeviceDirection PlayRecDir ){
		/**
	...
	void DevicesManagerNew:: connectSignalsToSlots(){
		...
		// playback devices
		connect( m_treeWidget_outputPorts, SIGNAL(itemDoubleClicked(QTreeWidgetItem*, int)),
				this, SLOT(slotOutputPortDoubleClicked(QTreeWidgetItem*, int)) );
		connect( m_treeWidget_outputPorts, SIGNAL(itemClicked(QTreeWidgetItem*, int)),
				this, SLOT(slotOutputPortClicked(QTreeWidgetItem*, int)) );
		...
		connect( m_treeWidget_inputPorts, SIGNAL(itemDoubleClicked(QTreeWidgetItem*, int)),
				this, SLOT(slotInputPortDoubleClicked(QTreeWidgetItem*, int)) );
		connect( m_treeWidget_inputPorts, SIGNAL(itemClicked(QTreeWidgetItem*, int)),
				this, SLOT(slotInputPortClicked(QTreeWidgetItem*, int)) );
		...
		connect(pushButton_newRecordDevice, SIGNAL(clicked()), this, SLOT(slotAddRecordDevice()));
		
		connect(pushButton_deletePlaybackDevice, SIGNAL(clicked()), this, SLOT(slotDeletePlaybackDevice()));
		connect(pushButton_deleteRecordDevice, SIGNAL(clicked()), this, SLOT(slotDeleteRecordDevice()));
